...
so verify a failure manually before removing a link.

### Sign-off on commits

From `.github/CONTRIBUTING.md`. Commits should include a `signed-off-by` line.
To automate this, add to `.git/hooks/prepare-commit-msg`:
...
- `README.md` mixes Markdown with raw HTML (`<p align="center">`, `<h4>`,
  `<img>`, etc.) and GitHub emoji shortcodes (e.g.
  `:notebook_with_decorative_cover:`). Preserve both when editing.
...
- Per `README.md`, a URL marked `*` is temporarily unavailable. Do not delete
  such links without confirming the resource is permanently gone.
