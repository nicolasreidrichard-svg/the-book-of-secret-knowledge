...
- Do not delete URLs marked `*` (temporarily unavailable) without confirming
  they are permanently expired — see `README.md`.
- See `.github/CONTRIBUTING.md` for full guidance.

## TODO

- No CI workflows are defined under `.github/workflows/` on `master`; document
  any that land (an EPUB/PDF generation workflow has been proposed on a
